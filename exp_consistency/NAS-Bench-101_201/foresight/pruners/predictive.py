...
def find_measures_arrays(net_orig, trainloader, dataload_info, device, pre_data_pair=None, measure_names=None, loss_fn=F.cross_entropy, **kwargs):
    ...
    if dataload == 'random':
        if pre_data_pair is not None:
            inputs, targets = pre_data_pair
        else:
        ...
        try:
            for measure_name in measure_names:
                if measure_name not in measure_values:
                    if measure_name in ["zico"]:
                        val = measures.calc_measure(measure_name, net_orig, device, trainloader, loss_fn=loss_fn)
                    elif measure_name in ["effective_zico"]:
                        val = measures.calc_measure(measure_name, net_orig, device, trainloader, loss_fn=loss_fn, THR1=kwargs['THR1'], THR2=kwargs['THR2'])
                    elif measure_name in ["effective_capacity", "effective_gradnorm", "effective_snip", "effective_grasp", "effective_ntktrace", "effective_gradsign"]:
                        val = measures.calc_measure(measure_name, net_orig, device, inputs, targets, loss_fn=loss_fn, THR1=kwargs['THR1'], THR2=kwargs['THR2'])
                    else:
...
def find_measures(net_orig,                  # neural network
    ...
    if measures_arr is None:
        measures_arr = find_measures_arrays(net_orig, dataloader, dataload_info, device, pre_data_pair=pre_data_pair, loss_fn=loss_fn, measure_names=measure_names, **kwargs)

    measures = {}
    for k,v in measures_arr.items():
        if k in ['jacob_cov', 'grad_conflict', 'grad_angle', 'size', 'params', 'flops', 'te_score', 'zen_score', 'ntktrace', 'effective_capacity', 'effective_capacity2', 'effective_gradnorm', 'effective_snip', 'effective_grasp', 'effective_ntktrace', 'zico', 'effective_zico', 'effective_gradsign']:
            measures[k] = v
        else:
